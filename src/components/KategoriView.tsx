  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-extrabold text-slate-900 font-display">Kategori</h2>
          <p className="text-sm text-slate-500 mt-0.5">Kelola tugas berdasarkan kategori</p>
        </div>
        <button onClick={onAddTask}
          className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold rounded-xl shadow-lg shadow-blue-200 transition-all cursor-pointer">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        {CATEGORIES.map(({ key, emoji, color, bg, border, text }) => {
          ...
          return (
            <button key={key}
              ...
              className={`p-4 rounded-2xl border text-left transition-all cursor-pointer ${
                openCat === key
                  ? `${bg} ${border} ring-2 ring-offset-1 ${text}`
            ...
            >
              ...
              <p className="text-xs text-slate-400 font-medium mt-0.5">{catTasks.length} tugas</p>
              {/* Progress bar */}
              <div className="mt-2.5 h-1 bg-slate-200 rounded-full overflow-hidden">
                <div className={`h-full ${color} rounded-full transition-all duration-500`} style={{ width: `${pct}%` }} />
              </div>
              <p className={`text-[10px] font-bold mt-1 ${pct === 100 && catTasks.length > 0 ? 'text-emerald-500' : 'text-slate-400'}`}>
                {pct}% selesai
              </p>
            </button>
          );
      ...
      {openCat && (() => {
        ...
        return (
          <div className="bg-white rounded-3xl border border-slate-200/60 shadow-xs p-5 sm:p-6 space-y-5">
            {/* Category header */}
            <div className="flex items-center gap-3 pb-4 border-b border-slate-100">
              <div className={`w-10 h-10 ${cat.bg} ${cat.border} border rounded-2xl flex items-center justify-center text-xl`}>
                {cat.emoji}
              </div>
              <div>
                <h3 className={`text-base font-extrabold ${cat.text}`}>{openCat}</h3>
                <p className="text-xs text-slate-400">{catTasks.length} tugas · {doneTasks.length} selesai</p>
              </div>
              <div className="ml-auto flex gap-2 text-center">
                <div>
                  <p className={`text-xl font-extrabold ${cat.text}`}>{activeTasks.length}</p>
                  <p className="text-[10px] text-slate-400">Aktif</p>
                </div>
                <div className="w-px bg-slate-100" />
                <div>
                  <p className="text-xl font-extrabold text-emerald-600">{doneTasks.length}</p>
            ...
            {catTasks.length === 0 ? (
              <div className="py-12 text-center">
                ...
                <button onClick={onAddTask}
                  className="mt-3 inline-flex items-center gap-1.5 px-4 py-2 text-xs font-bold text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-xl transition-colors cursor-pointer">
              ...
              <div className="space-y-5">
                ...
                {activeTasks.length > 0 && (
                  <div>
                    <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-2.5">Belum Selesai ({activeTasks.length})</p>
                    <div className="space-y-2">
                      {activeTasks.map(task => (
                        <div key={task.id}
                          className="flex items-center gap-3 p-3.5 bg-slate-50 border border-slate-100 rounded-2xl hover:border-slate-200 transition-all group">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              ...
                              {task.synced && <span className="text-[9px] font-bold text-blue-500 bg-blue-50 px-1.5 py-0.5 rounded-full">GCal</span>}
                            </div>
                          </div>
                          <button onClick={() => onEdit(task)}
                            className="p-1.5 text-slate-300 hover:text-blue-500 opacity-0 group-hover:opacity-100 rounded-lg transition-all cursor-pointer shrink-0">
                ...
                {doneTasks.length > 0 && (
                  <div>
                    <p className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-2.5">Selesai ({doneTasks.length})</p>
                    <div className="space-y-2">
                      {doneTasks.map(task => (
                        <div key={task.id}
                          className="flex items-center gap-3 p-3.5 bg-slate-50/50 border border-slate-100/60 rounded-2xl opacity-60 group">
                          <button onClick={() => onToggleComplete(task)} className="shrink-0 cursor-pointer focus:outline-none">
                            <CheckCircle2 className="h-4 w-4 text-emerald-500" />
                          </button>
                          <div className="flex-1 min-w-0">
                            <p className="text-xs font-bold text-slate-500 line-through truncate">{task.title}</p>
