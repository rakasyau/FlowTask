...
const CATEGORIES: { key: TaskCategory; emoji: string; color: string; bg: string; text: string }[] = [
  { key: 'Pekerjaan', emoji: '💼', color: 'bg-blue-500',    bg: 'bg-blue-50',    text: 'text-blue-700'   },
  { key: 'Pribadi',   emoji: '🏠', color: 'bg-rose-500',    bg: 'bg-rose-50',    text: 'text-rose-700'   },
  { key: 'Belajar',   emoji: '📚', color: 'bg-violet-500',  bg: 'bg-violet-50',  text: 'text-violet-700' },
  { key: 'Kesehatan', emoji: '❤️', color: 'bg-emerald-500', bg: 'bg-emerald-50', text: 'text-emerald-700'},
  { key: 'Lainnya',   emoji: '📋', color: 'bg-slate-400',   bg: 'bg-slate-50',   text: 'text-slate-600'  },
];
...
export default function StatistikView({ tasks }: StatistikViewProps) {
  ...
  return (
    <div className="space-y-5">
      {/* Header */}
      <div>
        <h2 className="text-2xl font-extrabold text-slate-900 font-display">Statistik</h2>
        <p className="text-sm text-slate-500 mt-0.5">Pantau produktivitas dan perkembangan Anda</p>
      </div>

      {/* Top KPI cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { icon: Target,      label: 'Produktivitas',    value: `${stats.score}%`,        sub: `${stats.completed} dari ${stats.total} tugas`, color: 'text-blue-600',    bg: 'bg-blue-50'    },
          { icon: CheckCircle2,label: 'Total Selesai',    value: stats.completed,           sub: `${stats.active} masih aktif`,                  color: 'text-emerald-600', bg: 'bg-emerald-50' },
          { icon: Clock,       label: 'Jam Produktif',   value: `${stats.totalHours}j`,    sub: 'dari tugas yang selesai',                      color: 'text-violet-600',  bg: 'bg-violet-50'  },
          { icon: Flame,       label: 'Streak Harian',   value: `${stats.streak} hari`,    sub: streak_label(stats.streak),                     color: 'text-orange-500',  bg: 'bg-orange-50'  },
        ].map(({ icon: Icon, label, value, sub, color, bg }) => (
          <div key={label} className="bg-white rounded-2xl border border-slate-200/60 shadow-xs p-4">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5">
        ...
        <div className="xl:col-span-8 bg-white rounded-3xl border border-slate-200/60 shadow-xs p-5 sm:p-6">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h3 className="text-sm font-extrabold text-slate-900">Aktivitas 14 Hari Terakhir</h3>
              <p className="text-[11px] text-slate-400 mt-0.5">Jumlah tugas per hari</p>
            </div>
          ...
          {stats.total === 0 ? (
            <div className="py-12 text-center">
              ...
              <p className="text-sm text-slate-400">Belum ada data aktivitas</p>
            </div>
          ) : (
              ...
              {stats.days14.map((day) => (
                <div key={day.date} className="flex-1 flex flex-col items-center gap-1 group">
                  <div className="relative w-full flex flex-col items-center justify-end h-28">
                    {/* Total bar */}
                    <div
                      className="w-full bg-blue-100 rounded-t-lg transition-all duration-500 relative overflow-hidden"
                      style={{ height: `${(day.total / maxDay) * 100}%`, minHeight: day.total > 0 ? '4px' : '0' }}
                    >
                      {/* Completed overlay */}
                      {day.completed > 0 && (
                        <div
                          className="absolute bottom-0 left-0 right-0 bg-emerald-400 rounded-t-lg transition-all"
                  ...
                  {/* Tooltip on hover */}
                  <div className="opacity-0 group-hover:opacity-100 transition-opacity absolute -mt-16 bg-slate-900 text-white text-[9px] px-2 py-1 rounded-lg pointer-events-none whitespace-nowrap z-10">
          ...
          <div className="flex justify-between mt-1 px-0.5">
            <span className="text-[9px] text-slate-300">{stats.days14[0]?.label}</span>
            <span className="text-[9px] text-slate-300">{stats.days14[13]?.label}</span>
          </div>
        </div>
        ...
        <div className="xl:col-span-4 space-y-4">
          ...
          {/* Quick stats */}
          <div className="bg-white rounded-3xl border border-slate-200/60 shadow-xs p-4 space-y-3">
            <h4 className="text-xs font-extrabold text-slate-500 uppercase tracking-widest">Ringkasan</h4>
            {[
      ...
      {/* ── Category breakdown ── */}
      <div className="bg-white rounded-3xl border border-slate-200/60 shadow-xs p-5 sm:p-6 space-y-4">
        <h3 className="text-sm font-extrabold text-slate-900">Distribusi per Kategori</h3>
              ...
              <div key={key} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-[10px] text-slate-400 font-medium">{completed}/{total} selesai</span>
                  </div>
                  <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                    <div className={`h-full ${color} rounded-full transition-all duration-500`} style={{ width: `${pct}%` }} />
                  </div>
                </div>
                <span className="text-xs font-extrabold text-slate-600 w-10 text-right shrink-0">{pct}%</span>
              </div>
            );
          })}
        </div>
      ...
      {stats.total > 0 && stats.topCat.completed > 0 && (
        <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-3xl p-5 flex items-center gap-4">
          ...
          <div>
            <p className="text-[10px] font-extrabold text-amber-600 uppercase tracking-widest flex items-center gap-1">
              <Award className="h-3 w-3" /> Kategori Terprodukif
            </p>
            <p className="text-sm font-extrabold text-slate-900 mt-0.5">{stats.topCat.key}</p>
            <p className="text-xs text-slate-500">{stats.topCat.completed} tugas selesai dari {stats.topCat.total}</p>
          </div>
        </div>
      )}
    </div>
